         ...
         {win.team===""?null:<h1 className="text-xl font-semibold tracking-wide">Team: {win.team}</h1>}
            ...
            {win.members.map((member, subIndex) => (
              ...
              <div  className="flex flex-row items-center justify-evenly w-[300px]">
                <img src={member.image} alt={member.name} className="w-[70px] h-[100px] object-cover rounded-full" />
                <h5 className="text-xl">{member.name}</h5>
                
              </div>
              <div className="flex flex-row items-center gap-3">
                <a className="border-2 border-blue-400 bg-blue-700 px-2 py-1 rounded-xl" href={member.social}>S1</a>
                <a href={member.link} className="border-2 border-blue-400 bg-blue-700 px-2 py-1 rounded-xl">S2</a>
              </div>
              </div>
            ))}
          </div>
